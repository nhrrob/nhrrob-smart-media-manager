...
export default function DetailsPanel( { fileId } ) {
	...
	useEffect( () => {
		...
		return () => document.removeEventListener( 'nhrsmm:focus-folder-select', handler );
	}, [] );
	...
	async function generateAlt() {
		if ( ! cfg.aiConfigured ) {
			showToast( 'No AI provider configured. Go to Settings → Connectors.', 'warning' );
			return;
		}
		setAiState( 'loading' );
		try {
			const res = await post( '/ai/alt-text', { attachment_id: fileId } );
			setAiState( { altText: res.alt_text, chars: res.chars || res.alt_text.length, model: res.model, latency: res.latency } );
		} catch ( e ) {
		...
		return (
			<aside className="smm-details" style={ { display: 'flex' } }>
				<div className="details-header">
					...
					<button className="btn-icon" onClick={ () => dispatch( { type: 'CLEAR_SELECTION' } ) }>
						<i className="ti ti-x" />
					</button>
				</div>
				<div className="details-preview" style={ { display: 'flex', alignItems: 'center', justifyContent: 'center', height: '140px' } }>
		...
		<aside className="smm-details" style={ { display: 'flex' } }>
			...
			<div className="details-preview">
				{ file.type === 'image' && file.thumb ? (
					<img src={ file.full_url || file.thumb } alt={ file.title } style={ { maxWidth: '100%', maxHeight: '160px', objectFit: 'contain' } } />
				) : (
			...
			<div className="details-body">
				...
				{ file.type === 'image' && (
					<div className="details-field">
						...
						<textarea
							...
							rows="2"
							value={ alt }
							onChange={ e => { setAlt( e.target.value ); scheduleSave(); } }
						/>
					</div>
				) }
				...
				<div className="details-field">
					...
					<textarea
						...
						rows="2"
						value={ desc }
						onChange={ e => { setDesc( e.target.value ); scheduleSave(); } }
					/>
				</div>

				{/* Meta */}
				<div className="details-meta">
					<div className="details-meta-row"><span>File</span><b className="u-mono">{ file.filename }</b></div>
					<div className="details-meta-row"><span>Size</span><b>{ formatBytes( file.size ) }</b></div>
					{ file.width && <div className="details-meta-row"><span>Dims</span><b>{ file.width }×{ file.height }</b></div> }
					<div className="details-meta-row"><span>Date</span><b>{ formatDate( file.date ) }</b></div>
					<div className="details-meta-row"><span>By</span><b>{ file.author }</b></div>
				</div>
			...
			<div className="details-actions">
				<div className="action-btn-row">
					<button
						...
						onClick={ async () => { await copyToClipboard( file.url ); showToast( 'URL copied!', 'success' ); } }
					>
					...
					{ file.type === 'image' ? (
						<button
							className="btn btn-sm btn-default"
							onClick={ () => window.open( `${ cfg.adminUrl }post.php?post=${ file.id }&action=edit`, '_blank' ) }
						>
							<i className="ti ti-edit" /> Edit Image
						</button>
					) : (
						<a className="btn btn-sm btn-default" href={ file.url } download>
				...
				{ file.type === 'image' && (
					<div className="action-btn-row">
						<button className="btn btn-sm btn-ai" onClick={ generateAlt }>
				...
				{ aiState === 'loading' && (
					<div className="ai-suggestion-card is-loading">
						<div className="ai-card-header"><i className="ti ti-sparkles" /> Generating…</div>
						<div className="ai-card-body">
							<div className="ai-shimmer" style={ { width: '90%' } } />
							<div className="ai-shimmer" style={ { width: '75%' } } />
							<div className="ai-shimmer" style={ { width: '50%' } } />
				...
				{ aiState && aiState !== 'loading' && ! aiState.error && (
					<div className="ai-suggestion-card">
						<div className="ai-card-header"><i className="ti ti-sparkles" /> Suggestion ready</div>
						<div className="ai-card-body">
							<p className="ai-result-text">{ aiState.altText }</p>
							<div className="ai-result-meta">
								<span>{ aiState.chars } chars</span>
								<span>{ aiState.model || 'wp-ai-client' }</span>
								{ aiState.latency && <span>{ aiState.latency }ms</span> }
							</div>
						</div>
						<div className="ai-card-actions">
							<button className="btn btn-sm btn-primary" onClick={ acceptAlt }>Accept</button>
							<button className="btn btn-sm btn-default" onClick={ editAlt }>Edit</button>
							<button className="btn btn-sm btn-default" onClick={ () => setAiState( null ) }>Reject</button>
						</div>
					</div>
				) }
				{ aiState?.error && (
					<div className="ai-suggestion-card">
						<div className="ai-card-body">
							<div className={ `smm-notice smm-notice-${ aiState.errorType }` } style={ { margin: 0 } }>
			...
			<div className="details-usage">
				...
				{ usage !== null && usage.length > 0 && usage.slice( 0, 5 ).map( ( u, i ) => (
