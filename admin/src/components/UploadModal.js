...
export default function UploadModal() {
	const { dispatch, loadMedia, loadFolders, state: { uploadInitialFiles, folders } } = useApp();
	...
	function addFiles( files ) {
		...
		fileArr.forEach( file => {
			...
			setQueue( prev => [ ...prev, { id, name: file.name, icon, progress: 0, status: 'waiting' } ] );
			uploadFile( file, id );
		} );
	}
	...
	useEffect( () => {
		...
		const allDone = queue.every( item => item.status === 'ok' || item.status === 'fail' );
		setDone( allDone );
	}, [ queue ] );
	...
	return (
		<div className="smm-modal-overlay" style={ { display: 'flex' } } onClick={ close }>
			<div className="smm-modal" onClick={ e => e.stopPropagation() }>
				...
				<div className="modal-body">
					...
					<div className="upload-options" style={ { marginTop: '12px' } }>
						...
						<select className="smm-select" id="smm-upload-folder-select">
					...
					{/* Upload queue */}
					{ queue.length > 0 && (
						<div className="upload-queue" style={ { display: 'block' } }>
							{ queue.map( item => (
								<div key={ item.id } className="upload-item">
									<i className={ `ti ${ item.icon } upload-item-icon` } />
									...
									<span className={ `upload-item-status${ item.status === 'ok' ? ' success' : item.status === 'fail' ? ' error' : '' }` }>
										{ item.status === 'ok' ? '✓' : item.status === 'fail' ? '✗' : 'Waiting…' }
									</span>
								</div>
							) ) }
						</div>
				...
				{ done && (
					<div className="modal-footer" style={ { display: 'flex' } }>
						<span className="upload-summary">
							{ results.fail > 0 ? (
								<span className="smm-notice smm-notice-warning" style={ { padding: '4px 8px', borderRadius: '4px' } }>
									{ results.ok } uploaded, { results.fail } failed
								</span>
							) : (
								<span className="smm-notice smm-notice-success" style={ { padding: '4px 8px', borderRadius: '4px' } }>
									{ results.ok } file{ results.ok === 1 ? '' : 's' } uploaded successfully
								</span>
							) }
						</span>
						<button className="btn btn-default" onClick={ onDone }>Done</button>
					</div>
				) }
			</div>
		</div>
