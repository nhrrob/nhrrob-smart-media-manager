...
export default function Sidebar() {
	...
	useEffect( () => {
		...
		return () => document.removeEventListener( 'nhrsmm:rename-folder', onAction );
	}, [] );

	// Count all files across all folders
	function sumCount( arr ) {
		return arr.reduce( ( acc, f ) => acc + ( f.count || 0 ) + sumCount( f.children || [] ), 0 );
	}
	...
	return (
		<aside className="smm-sidebar" id="smm-sidebar">
			...
			<div className="sidebar-section">
				...
				<div
					className={ `nav-item${ currentFolder === null ? ' active' : '' }` }
				...
				<div
					className={ `nav-item${ currentFolder === 0 ? ' active' : '' }` }
			...
			<div className="sidebar-section">
				<div className="sidebar-section-label" style={ { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } }>
					...
					>
						<i className="ti ti-folder-plus" style={ { fontSize: '14px', color: 'var(--gray-400)', cursor: 'pointer' } } />
					</button>
				</div>
				...
				<div id="smm-folder-tree">
					{ folders.length === 0 ? (
						<div style={ { padding: '4px 8px', fontSize: '11px', color: 'var(--gray-400)', fontStyle: 'italic' } }>
							No folders yet
						</div>
					) : (
						<FolderList
...
function FolderList( { folders, depth, openFolders, currentFolder, renamingId, onNavigate, onToggleOpen, onRename, onCancelRename, onStartRename, loadFolders, loadMedia, showToast, dispatch } ) {
...
/* ── Single folder item ────────────────────────────────────── */
function FolderItem( { folder: f, depth, openFolders, currentFolder, renamingId, onNavigate, onToggleOpen, onRename, onCancelRename, onStartRename, loadFolders, loadMedia, showToast, dispatch } ) {
	...
	function onDragStart( e ) {
		...
		e.dataTransfer.setData( 'text/plain', JSON.stringify( { folderDrag: true, folderId: f.id } ) );
	}
	...
	async function onDrop( e ) {
		...
		try {
			...
			if ( data.folderDrag ) {
				...
				await post( `/folders/${ data.folderId }/move`, { parent: f.id } );
				await loadFolders();
			} else if ( data.fileIds ) {
				await post( '/media/bulk-move', { ids: data.fileIds, folder_id: f.id } );
				showToast( `Moved ${ data.fileIds.length } file${ data.fileIds.length === 1 ? '' : 's' } to folder.`, 'success' );
				await loadFolders();
				loadMedia();
			}
		} catch ( err ) {
	...
	function onContextMenu( e ) {
		e.preventDefault();
		dispatch( { type: 'SHOW_CONTEXT_MENU', kind: 'folder', id: f.id, x: e.clientX, y: e.clientY } );
	}
	...
	return (
		<>
			...
			>
				{ hasKids ? (
					<i
						className={ `ti ti-chevron-${ isOpen ? 'down' : 'right' } folder-chevron` }
						style={ { fontSize: '12px', color: 'var(--gray-300)', flexShrink: 0, cursor: 'pointer' } }
					/>
				) : (
				...
				{ renaming ? (
					<input
						...
						defaultValue={ f.name }
						onKeyDown={ e => {
							if ( e.key === 'Enter' ) onRename( f.id, e.target.value.trim(), f.name );
							if ( e.key === 'Escape' ) onCancelRename();
						} }
						onBlur={ e => onRename( f.id, e.target.value.trim(), f.name ) }
